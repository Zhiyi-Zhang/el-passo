...
def generate_params_for_RP():
    ...
    print(len(json_param['el_cipher_1']))
    print(len(json_param['el_cipher_2']))
    for i in range(hidden_elements):
    ...
    async with session.get(url, json=json_param) as response:
        if measurement:
            ...
            for item in response.raw_headers:
                total_len += len(item[0])
                total_len += len(item[1])
            print("response header length: ", str(total_len))
...
if __name__ == '__main__':
    # usage: python3 user.py idp_IP rp_IP hide_additional_element_num total_running_times_of_login
    if len(sys.argv) >= 3:
    ...
    if len(sys.argv) >= 5:
        running_times = int(sys.argv[4].rstrip())
