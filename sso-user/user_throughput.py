...
async def main():
    ...
    for i in range(running_times_idp):
        ...
        time_to_sleep = idp_interval - (idp_end_time - idp_start_time)
        if(time_to_sleep > 0):
            time.sleep(time_to_sleep)

    # communicate with RP
    json_param = generate_params_for_RP()
    for i in range(running_times_rp):
        ...
        time_to_sleep = rp_interval - (rp_end_time - rp_start_time)
        if(time_to_sleep > 0):
            time.sleep(time_to_sleep)
if __name__ == '__main__':
    ...
    if len(sys.argv) >= 4:
        hidden_elements = int(sys.argv[3].rstrip())

    if len(sys.argv) >= 5:
        running_times_idp = int(sys.argv[4].rstrip())

    if len(sys.argv) >= 6:
        idp_interval = float(sys.argv[5].rstrip())

    if len(sys.argv) >= 7:
        running_times_rp = int(sys.argv[6].rstrip())
    if len(sys.argv) >= 8:
        rp_interval = float(sys.argv[7].rstrip())
